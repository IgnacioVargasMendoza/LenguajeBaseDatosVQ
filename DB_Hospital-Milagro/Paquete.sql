...
-- Declaración de procedimientos públicos
  ...
  PROCEDURE FIDE_AGREGAR_INVENTARIO_MEDICAMENTO_SP;
  ...
  PROCEDURE FIDE_REGISTRAR_HISTORIAL_CITA_SP;
  PROCEDURE FIDE_CREAR_CITA_SP;
  PROCEDURE FIDE_AUTENTICAR_USUARIO_SP;
  ...
  PROCEDURE actualizar_citas_vencidas;
  PROCEDURE generar_reporte_medicamentos;
  PROCEDURE transferir_inventario;
  PROCEDURE consolidar_tratamientos_paciente;
  PROCEDURE migrar_historial_citas_antiguas;
  PROCEDURE FIDE_LISTAR_USUARIOS_SP;
  PROCEDURE reset_pass_inactivos;
  PROCEDURE eliminar_usuarios_sin_rol;
  PROCEDURE actualizar_licencias_temp;
  PROCEDURE desactivar_doctores_inactivos;
  PROCEDURE asignar_direccion_default;
  PROCEDURE estandarizar_telefonos;
  PROCEDURE cancelar_citas_antiguas;
  PROCEDURE notificar_citas_proximas;
  PROCEDURE aplicar_recargo_mora;
  PROCEDURE archivar_facturas_antiguas;
  PROCEDURE actualizar_precios_medicamentos;
  PROCEDURE desactivar_medicamentos_sin_stock;
  PROCEDURE normalizar_nombres_paises;
  PROCEDURE eliminar_distritos_no_usados;
  PROCEDURE prefijar_especialidades;
  PROCEDURE actualizar_descripciones_vacias;
  PROCEDURE finalizar_tratamientos_antiguos;
  PROCEDURE completar_descripciones_tratamientos;
  PROCEDURE purgar_notificaciones_antiguas;
  PROCEDURE desactivar_notificaciones_leidas;
  PROCEDURE reporte_doctores_especialidades;
  PROCEDURE reporte_pacientes_tratamientos;
  PROCEDURE verificar_inventario_bajo;
  PROCEDURE validar_direcciones;
  PROCEDURE estadisticas_citas_paciente;
  PROCEDURE top_medicamentos_usados;
  PROCEDURE balancear_carga_doctores;
  PROCEDURE buscar_paciente;
  PROCEDURE reporte_citas_rango;
  PROCEDURE transferir_pacientes;
  PROCEDURE reasignar_citas_paciente;
  PROCEDURE listar_medicamentos_stock_critico;
  PROCEDURE reporte_facturas_pendientes;
  PROCEDURE promedio_citas_doctor;
  PROCEDURE buscar_citas_por_fecha_especialidad;
  PROCEDURE actualizar_precios_medicamentos_porcentaje;
  PROCEDURE listar_tratamientos_procedimientos;
  PROCEDURE verificar_disponibilidad_doctores;
  PROCEDURE resumen_mensual_citas;
  PROCEDURE migrar_direcciones_antiguas;
  PROCEDURE FIDE_LISTAR_USUARIOS_SP;
  PROCEDURE FIDE_LISTAR_DOCTORES_SP;
  PROCEDURE FIDE_LISTAR_ESPECIALIDADES_SP;
...
CREATE OR REPLACE PROCEDURE FIDE_ACTUALIZAR_TB_SP(
    p_id    SOME_TABLE.id%TYPE,
    p_nombre SOME_TABLE.nombre%TYPE,
    p_id_pais SOME_TABLE.id_pais%TYPE
) AS
BEGIN
    UPDATE SOME_TABLE
    SET NOMBRE = p_nombre,
...
var LISTA_DOCTORES REFCURSOR;
EXEC FIDE_LISTAR_DOCTORES_SP(:LISTA_DOCTORES);
PRINT LISTA_DOCTORES;


SELECT * FROM FIDE_DOCTORES_TB
/
